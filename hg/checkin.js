...
class CheckinManager {
    ...
    searchBookings() {
        ...
        // If no bookings exist, offer to generate sample data
        if (bookings.length === 0) {
            this.displayNoBookingsMessage();
            return;
        }
    ...
    displaySearchResults(bookings) {
        ...
        if (bookings.length === 0) {
            const guestSearch = document.getElementById('guestSearch').value;
            const bookingIdSearch = document.getElementById('bookingIdSearch').value;
            const roomSearch = document.getElementById('roomSearch').value;
            
            let searchCriteria = [];
            if (guestSearch) searchCriteria.push(`Guest: "${guestSearch}"`);
            if (bookingIdSearch) searchCriteria.push(`Booking ID: "${bookingIdSearch}"`);
            if (roomSearch) searchCriteria.push(`Room: "${roomSearch}"`);
            
            const searchText = searchCriteria.length > 0 ? ` for ${searchCriteria.join(', ')}` : '';
            
            // Get all available bookings for debugging
            const allBookings = JSON.parse(localStorage.getItem('hotelBookings')) || [];
            const availableIds = allBookings.map(b => `#${b.id}`).join(', ');
            ...
            resultsList.innerHTML = `
                <div class="text-center py-8">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900 mb-2">No Bookings Found${searchText}</h3>
                    <p class="text-gray-600 mb-4">Try adjusting your search criteria or check if the booking exists.</p>
                    
                    ${allBookings.length > 0 ? `
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                            <h4 class="font-medium text-blue-900 mb-2">Available Booking IDs:</h4>
                            <p class="text-sm text-blue-800">${availableIds}</p>
                        </div>
                    ` : ''}
                    
                    <div class="space-y-3">
                        <button onclick="checkinManager.clearSearch()" 
                                class="bg-blue-600 text-white px-6 py-2 rounded-md hover:bg-blue-700">
        ...
        const checkedInBookings = bookings.filter(b => b.status === 'checked_in' || b.status === 'checked-in').length;
        const availableForCheckin = bookings.filter(b => {
            const isAlreadyCheckedIn = b.status === 'checked_in' || b.status === 'checked-in';
            const status = this.getBookingStatus(b);
            return !isAlreadyCheckedIn && (status === 'Active' || status === 'Upcoming');
        }).length;
        ...
        const statsHeader = `
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-3 mb-4">
                <div class="grid grid-cols-3 gap-4 text-center">
                    <div>
                        <div class="text-lg font-semibold text-gray-900">${totalBookings}</div>
                        <div class="text-sm text-gray-600">Total Bookings</div>
                    </div>
                    <div>
                        <div class="text-lg font-semibold text-green-600">${availableForCheckin}</div>
                        <div class="text-sm text-gray-600">Available for Check-in</div>
                    </div>
                    <div>
                        <div class="text-lg font-semibold text-blue-600">${checkedInBookings}</div>
        ...
        resultsList.innerHTML = statsHeader + bookings.map(booking => {
            ...
            // Check if this booking is already checked in
            const isAlreadyCheckedIn = booking.status === 'checked_in' || booking.status === 'checked-in';
            const canCheckIn = !isAlreadyCheckedIn && (status === 'Active' || status === 'Upcoming');
                ...
                <div class="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
                    <div class="flex justify-between items-start">
                        ...
                        <div class="flex flex-col space-y-2 ml-4">
                            <button onclick="checkinManager.selectBooking('${booking.id}')" 
                                    class="bg-blue-600 text-white px-3 py-1 rounded text-sm hover:bg-blue-700">
                            Select
                        </button>
                            ${canCheckIn ? `
                                <button onclick="checkinManager.quickCheckin('${booking.id}')" 
    ...
    renderCheckinHistory() {
        ...
        if (tableBody) {
            tableBody.innerHTML = this.checkins.map(checkin => this.createCheckinRow(checkin)).join('');
        }
    ...
    updateSummaryStatistics() {
        ...
        const activeCheckins = this.checkins.filter(c => c.status === 'checked-in').length;
        const completedCheckouts = this.checkins.filter(c => c.status === 'checked-out').length;
        
        // Calculate total revenue from checkouts
        const checkouts = JSON.parse(localStorage.getItem('checkouts') || '[]');
        const totalRevenue = checkouts.reduce((sum, checkout) => sum + (checkout.finalAmount || 0), 0);
        ...
        document.getElementById('completedCheckouts').textContent = completedCheckouts;
        document.getElementById('totalRevenue').textContent = `$${totalRevenue.toFixed(2)}`;
    }

    createCheckinRow(checkin) {
        const statusClass = this.getStatusBadgeClass(checkin.status);
        const statusText = this.getStatusText(checkin.status);
        
        // Load checkout data to see if this check-in has been checked out
        const checkouts = JSON.parse(localStorage.getItem('checkouts') || '[]');
        const checkout = checkouts.find(c => c.checkinId === checkin.id);
        
        // Generate checkout info display
        let checkoutInfo = '';
        if (checkout) {
            checkoutInfo = `
                <div class="text-xs">
                    <div class="font-medium text-green-600">Checked Out</div>
                    <div class="text-gray-500">${this.formatDateTime(checkout.checkoutTime)}</div>
                    <div class="text-gray-500">Final: $${checkout.finalAmount || 0}</div>
                </div>
            `;
        } else if (checkin.status === 'checked-out') {
            checkoutInfo = `
                <div class="text-xs">
                    <div class="font-medium text-yellow-600">Checked Out</div>
                    <div class="text-gray-500">No detailed info</div>
                </div>
            `;
        } else {
            checkoutInfo = `
                <div class="text-xs">
                    <div class="font-medium text-blue-600">Checked In</div>
                    <div class="text-gray-500">Active stay</div>
                </div>
            `;
        }
        ...
        return `
            <tr class="hover:bg-gray-50">
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${checkin.id}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${checkin.guestName}</td>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">$${checkin.totalAmount || 0}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    ${checkoutInfo}
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <button data-action="delete-checkin" data-id="${checkin.id}" 
                            class="text-red-600 hover:text-red-900 mr-3">
                        Delete
                    </button>
                    <button data-action="view-checkin" data-id="${checkin.id}" 
    ...
    clearSearch() {
        ...
        // Show all bookings instead of hiding results
        this.showAllBookings();
        
        // Show notification
        this.showNotification('Search cleared - showing all bookings', 'info');
    }
    ...
    processDirectCheckin() {
        ...
        // Find the booking
        const bookings = JSON.parse(localStorage.getItem('hotelBookings')) || [];
        const booking = bookings.find(b => b.id.toString() === cleanBookingId);
        
        if (!booking) {
            this.showNotification(`Booking ID #${cleanBookingId} not found. Please check the ID and try again.`, 'error');
            
            // Show available booking IDs for reference
            if (bookings.length > 0) {
