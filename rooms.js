...
class RoomsManager {
    ...
    bindEvents() {
        ...
        document.addEventListener('click', (e) => {
            ...
            } else if (e.target.matches('[data-action="mark-available"]')) {
                const roomId = parseInt(e.target.getAttribute('data-id'));
                this.markRoomAsAvailable(roomId);
            }
        });
    }
    ...
    createRoomCard(room) {
        ...
        card.innerHTML = `
            ...
            <div class="flex gap-2 mt-4 pt-4 border-t border-gray-200">
                ...
                ${room.status === 'cleaning' ? `<button data-action="mark-available" data-id="${room.id}" class="flex-1 bg-green-600 text-white px-3 py-2 rounded-md text-sm hover:bg-green-700 transition-colors">Mark as Available</button>` : ''}
            </div>
        `;
    ...
    markRoomAsAvailable(roomId) {
        ...
        if (index !== -1 && this.rooms[index].status === 'cleaning') {
            this.rooms[index].status = 'available';
            this.saveRooms();
            this.renderRooms();
            this.showNotification('Room marked as available!', 'success');
        }
    }
}
